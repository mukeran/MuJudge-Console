...
#pragma region warning
	...
	printf("2. 本程序默认比较模式为严格模式，即不忽略文件末尾换行，100%相等则 AC\n");
	printf("3. 其它比较模式将在下一个版本支持\n");
	printf("4. 以后版本会支持exe直接评测\n");
	printf("5. 时间测试可能不准确\n");
...
#pragma region config
	...
	配置部分
	*/
	char pathofgcc[MAXN] = { 0 }, formatofin[MAXN] = { 0 }, formatofout[MAXN] = { 0 }, tmpmsecond[MAXN] = { 0 };
	...
	//超时设置
	printfcolor(COLOR_CYAN, "\n请输入程序超时时间，以毫秒(ms)为单位，必须为整数，直接回车则为不限时间\n");
	...
	*/
	...
	while (true) {
		...
		if (rs == 1) {
			...
			string backupProgramOutput = programOutput;
			...
			fclose(fp);
			string backupStdOutput = stdOutput;
